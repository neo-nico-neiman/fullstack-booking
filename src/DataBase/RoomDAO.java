...
public class RoomDAO {
	...
	public Room getRoom(int roomNumber) {
		...
		try {
			...
			String query = "SELECT  r.is_available, rt.category_name, rt.price_per_night, rt.max_allowance, rt.image, rt.room_description, rt.room_details "
					+ "FROM rooms AS r JOIN room_type AS rt " + "ON r.room_category=rt.category_name"
					+ " WHERE r.room_number=" + roomNumber;
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery(query);
			rs.next();
			room.setRoomNumber(roomNumber);
			room.setRoomType(rs.getString("category_name"), rs.getInt("max_allowance"), rs.getDouble("price_per_night"),
					rs.getString("image"), rs.getString("room_description"), rs.getString("room_details"));
	...
	public void updateRoomNumber(int roomNumber, int newRoomNumber) {
		connect();
		try {
			String query = "UPDATE room SET room_number=? WHERE room_number=?";
			PreparedStatement pst = con.prepareStatement(query);
